...
class Conv_block(Module):  # verified: the same as ``Conv'' in ./fmobilefacenet
    def __init__(
        ...
        self.conv2d = Conv2d(
            ...
            padding=padding,
            bias=False,
        )  # verified: the same as mx.sym.Convolution(data=data, num_filter=num_filter, kernel=kernel, num_group=num_group, stride=stride, pad=pad, no_bias=True)
        self.batchnorm = BatchNorm2d(
            out_c, eps=0.001
        )  # verified: the same as mx.sym.BatchNorm(data=conv, fix_gamma=False,momentum=0.9)
...
class Depth_Wise(
    Module
):  # verified: if residual is False: the same as ``DResidual'' in ./fmobilefacenet; else: the same as ``Residual''
    def __init__(
        self,
        in_c,
        out_c,
...
class MobileFaceNet(Module):
    def __init__(self, embedding_size):
        ...
        self.fc1 = BatchNorm1d(
            embedding_size, eps=2e-5
        )  # doubt: the same as mx.sym.BatchNorm(data=conv_6_f, fix_gamma=True, eps=2e-5, momentum=0.9)?
...
class ModifiedMobileFaceNet(Module):
    def __init__(self, embedding_size):
        super(ModifiedMobileFaceNet, self).__init__()
        self.conv_1 = Conv_block(3, 64, kernel=(3, 3), stride=(2, 2), padding=(1, 1))
        self.conv_2_dw = Conv_block(
            64, 64, kernel=(3, 3), stride=(1, 1), padding=(1, 1), groups=64
        )
    ...
    def forward(self, x):
        ...
        out = self.res_5(out)
        out = self.conv_6sep(out)
        regressor_out = self.regressor(out)
        out = self.conv_6dw7_7(out)
        out = self.conv_6_flatten(out)
        out = self.pre_fc1(out)
        out = self.fc1(out)
        return out, regressor_out
