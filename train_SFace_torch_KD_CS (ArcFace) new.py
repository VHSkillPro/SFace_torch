...
if __name__ == "__main__":
    ...
    if MULTI_GPU:
        ...
        projection_layer = nn.DataParallel(projection_layer, device_ids=GPU_ID)
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for inputs, labels in iter(trainloader):
            ...
            student_features = projection_layer(features)
            hidden_rep_loss = torch.cosine_embedding_loss(
                student_features,
                teacher_features,
                torch.ones(features.size(0)).to(DEVICE),
            )
            ...
            if ((batch + 1) % DISP_FREQ == 0) and batch != 0:
                ...
                writer.add_scalar(
                    "CosineEmbeddingLoss", cosine_embedding_losses.avg, batch + 1
                )
                ...
                print(
                    ...
                    "CosineEmbeddingLoss {celoss.val:.5f} ({celoss.avg:.5f})".format(
                        ...
                        Wj=Wj_mean,
                        top1=top1,
                        celoss=cosine_embedding_losses,
                    )
                )
                # print("=" * 60)
                intra_losses = AverageMeter()
