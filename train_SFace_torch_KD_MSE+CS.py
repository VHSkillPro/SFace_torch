...
from backbone.model_mobilefacenet import MobileFaceNet, ModifiedMobileFaceNet
...
def xavier_normal_(tensor, gain=1.0, mode="avg"):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--stages", help="training stages", default="35,65,95", type=str
    )
    ...
    parser.add_argument(
        "--data_root",
        help="the dir for training",
        default="",
        type=str,
    )
    parser.add_argument(
        "--eval_path",
        help="the dir for validation",
        default="",
        type=str,
    )
    parser.add_argument(
        "--net",
        help="which network, ['IR_50', 'IR_101', 'MobileFaceNet']",
        default="MobileFaceNet",
        type=str,
    )
    parser.add_argument(
        "--head",
        help="head type, ['Softmax', 'ArcFace', 'CosFace', 'SphereFace', 'Am_softmax', 'SFaceLoss']",
        default="SFaceLoss",
        type=str,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--resume_backbone", help="resume backbone model", default="", type=str
    )
    parser.add_argument("--resume_head", help="resume head model", default="", type=str)
    parser.add_argument(
        "--teacher_backbone", help="teacher backbone model", type=str, required=True
    )
    ...
    BACKBONE_DICT = {
        ...
        "MobileFaceNet": ModifiedMobileFaceNet(EMBEDDING_SIZE),
    }
    ...
    if not os.path.isfile(teacher_backbone_path):
        raise ValueError(
    ...
    FEATURE_MAP_WEIGTH = 0.125
    HIDDEN_REP_LOSS_WEIGHT = 0.125
    SFACE_LOSS_WEIGHT = 1 - FEATURE_MAP_WEIGTH - HIDDEN_REP_LOSS_WEIGHT
    mse_loss = nn.MSELoss()
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for inputs, labels in iter(trainloader):
            ...
            features, regressor_feature_map = BACKBONE(inputs)

            with torch.no_grad():
                teacher_features, teacher_feature_map = teacher_backbone(inputs)

            hidden_rep_loss = mse_loss(regressor_feature_map, teacher_feature_map)
            hidden_rep_loss = hidden_rep_loss.mean()
            hidden_rep_loss_2 = torch.cosine_embedding_loss(
            ...
            total_loss = (
                FEATURE_MAP_WEIGTH * hidden_rep_loss
                + SFACE_LOSS_WEIGHT * loss
                + HIDDEN_REP_LOSS_WEIGHT * hidden_rep_loss_2
            )
